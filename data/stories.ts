...
import { asbLongReadingFrench } from "@/data/asbLongReadingFrench";
import { asbLongReadingTranslations } from "@/data/asbLongReadingTranslations";
...
type ReadingSection = {
  /** Amharic album title, shown as the section heading. */
  title: string;
  titleFr?: string;
  titleEn?: string;
  album: AlbumKey;
 ...
 */
function compose(sections: readonly ReadingSection[]) {
  const pages: string[] = [];
  const pageIllustrations: (StoryIllustration | undefined)[] = [];
  const fr: (string | undefined)[] = [];
  const en: (string | undefined)[] = [];

  for (const { title, titleFr, titleEn, album } of sections) {
    // The section divider is a heading, not a page of the album.
    pages.push(`ክፍል፦ ${title}`);
    pageIllustrations.push(undefined);
    fr.push(titleFr && `Section : ${titleFr}`);
    en.push(titleEn && `Section: ${titleEn}`);

    asbLongReadingPages[album].forEach((text, index) => {
      pages.push(text);
      pageIllustrations.push(asbLongReadingImages[album]?.[index]);
      fr.push(asbLongReadingFrench[album]?.[index]);
      en.push(asbLongReadingTranslations[album]?.[index]);
    });
  }

  return { pages, pageIllustrations, pageTranslations: { fr, en } };
}
...
export const stories: Story[] = [
  {
    ...
    ...compose([
      {
        title: melokuhleDay.title,
        titleFr: "Une journée dans la vie de Melokuhle",
        titleEn: "A day in the life of Melokuhle",
        album: "melokuhleDay",
      },
      {
        title: blueBus.title,
        titleFr: "Quand le grand bus bleu était en retard",
        titleEn: "When the big blue bus was late",
        album: "blueBus",
      },
      {
  ...
  },
  {
    ...
    ...compose([
      {
        title: crowdedHouse.title,
        titleFr: "L'homme qui avait un gros problème",
        titleEn: "Man with a serious problem",
        album: "crowdedHouse",
      },
      {
        title: birdKing.title,
        titleFr: "Le roi des oiseaux",
        titleEn: "King of the birds",
        album: "birdKing",
      },
      {
        title: mousePrince.title,
        titleFr: "Le fils du roi des rats",
        titleEn: "Rat king's son",
        album: "mousePrince",
      },
    ]),
    source: africanStorybookSource,
